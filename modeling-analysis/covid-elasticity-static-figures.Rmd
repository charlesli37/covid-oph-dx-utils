---
title: "Static Figures for the COVID Elasticity Paper"
author: "Charles Li"
date: "June 7, 2022"
...
```{r}
#### datasets
master_dx_df <- as.data.frame(read.csv('/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/shiny-app/dx_proportions_and_cnts.csv'))
master_dx_df$Y <- master_dx_df$num_dx_patients

#### codebooks 
icd_df <- as.data.frame(read.csv('/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/shiny-app/ccsr_codebook_for_sql.csv'))
icd_df$Diagnosis.Name <- stringr::str_trim(icd_df$DiagnosisEntity)
ccsr_cats <- read.csv('/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/shiny-app/CCSR categories 5.4.20_CLedits_01102021.csv', header = F)
...
severity_idx <- as.data.frame(read.csv('/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/shiny-app/base_score_modified_FLCLECedits_02162021.csv'))
...
vehss_idx <- read.csv('/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/shiny-app/VEHSS_VT_crosswalk.csv')
...
```{r}
write.csv(df_models, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/dx-models.csv')
```
...
#drop based on more strict PE metric (RMSPE <= 0.10)
accurate_dx_list2 <- df_models[df_models$RMSPE <= 0.10, 'Diagnosis Name']
```

```{r}
write.csv(accurate_dx_list, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/accurate-dx-list.csv')
```

```{r}
#for the supplement
write.csv(severity_idx[severity_idx$MAPPED_ENTITY_NAME %in% accurate_dx_list, ], '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/bourgesocularemerg-final.csv')
write.csv(vehss_idx[vehss_idx$DiagnosisEntity %in% accurate_dx_list, ], '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/vehssbig4-final.csv')
```

## Diagnosis codebook for MS
For manuscript supplement - export inventory of all diagnosis entities:
Do it category-by-category due to space considerations
...
for (i in 1:length(categoryList)){
  ...
  icd_ft <- theme_box(flextable(icd_df_exp))
  icd_ft2 <- merge_v(icd_ft, j = c('Diagnosis Entity', 'Diagnosis Category'))
  icd_ft_final <- valign(icd_ft2, j = 'Diagnosis Category', valign = 'top')
  
  read_docx() %>%
    body_add_flextable(value = icd_ft_final) %>%
    print(target = paste0("/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/icd-codebook-JAMA-", i, ".docx"))
...
### Deviances (with non-sigs as zero)
```{r}
...
#account for p > 0.05
obj3_p_mat <- as.matrix(obj3_p[, -1])
rownames(obj3_p_mat) <- obj3_p[, 1]

#convert to 1's and 0's:
obj3_p_mat_bin <- apply(obj3_p_mat, c(1,2), function(x) ifelse(x == "*", 1, 0))
obj3_mat2 <- as.data.frame(obj3_mat * obj3_p_mat_bin)

#keep only diagnoses with accurate enough RMSPEs
master_devs <- obj3_mat2[rownames(obj3_mat2) %in% accurate_dx_list, ]
```

```{r}
write.csv(master_devs, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-deviations.csv')
```

### Deviances (RAW, i.e., retaining original value of non-sigs)
```{r}
#keep only diagnoses with accurate enough RMSPEs
...
```{r}
write.csv(master_devsOrig, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-deviations-raw.csv')
```
...
```{r}
write.csv(master_dev_bounds, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-deviation-bounds.csv')
```
...
```{r}
write.csv(master_pvalues, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-pvalues.csv')
```

### Weighted deviances for heatmap shading
```{r}
master_devsWeighted <- -log(master_pvalues) * master_devsOrig
...
```{r}
write.csv(master_summarystats, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-summarystats.csv')
```
...
```{r}
write.csv(dx_legend_df, '/Users/charlesli/OneDrive - American Academy of Ophthalmology/covid-elasticity-paper/outputs/06072022/master-heatmap-legend.csv', row.names = F)
```
...
### 1A-1D: Time series of non-severe Big 4 eye conditions  
```{r}
eObjFig1a <- elasticities("dry AMD, early stage", master_dx_df)
fig1a_df <- as.data.frame(eObjFig1a[[8]])
fig1a_df$date <- as.Date(fig1a_df$date)
```
...
```{r}
eObjFig1b <- elasticities("NPDR w/o DME", master_dx_df)
fig1b_df <- as.data.frame(eObjFig1b[[8]])
fig1b_df$date <- as.Date(fig1b_df$date)
```
...
```{r}
eObjFig1c <- elasticities("cataract, age-related", master_dx_df)
fig1c_df <- as.data.frame(eObjFig1c[[8]])
fig1c_df$date <- as.Date(fig1c_df$date)
```
...
```{r}
eObjFig1d <- elasticities("glaucoma, suspect", master_dx_df)
fig1d_df <- as.data.frame(eObjFig1d[[8]])
fig1d_df$date <- as.Date(fig1d_df$date)
```

```{r}
fig1d <- ggplot(fig1d_df) +
  geom_line(aes(x = date, y = Y, col ='Observed')) +
  geom_line(aes(x = date, y = predicted, col = 'Predicted')) +
  geom_ribbon(aes(x = date, ymin = lowerPI, ymax = upperPI, fill = 'Predicted', color = 'Predicted'),  alpha = 0.15, show.legend = F) +
  geom_ribbon(aes(x = date, ymin = lowerPI, ymax = upperPI, fill = 'Predicted'), alpha = 0.15, show.legend = T) +
...
fig1 <- ggarrange(fig1a + rremove("axis.title"), fig1b + rremove("axis.title"), fig1c + rremove("axis.title"), fig1d + rremove("axis.title"), labels = c("A", "B", "C", "D"),
          common.legend = TRUE, 
          legend = "top")
annotate_figure(fig1, left = text_grob("Number of patients", rot = 90, size = 10), bottom = text_grob("Month of diagnosis documentation", size = 10))
```

### 1E: Heatmap of non-severe Big 4 eye conditions  

```{r}
col_fun_fig1e = colorRamp2(c(-4, 0, 4), c("#de425b", "#eeeeee", "#21a6db"))
col_fun_fig1e(seq(-3, 3))
```

```{r}
fig1edxlist <- c("glaucoma, suspect", "cataract, age-related", "NPDR w/o DME", "dry AMD, early stage")
fig1edata <- master_devsWeighted[rownames(master_devsWeighted) %in% fig1edxlist, c(1:24)]
fig1edataNums <- master_devsOrig[rownames(master_devsOrig) %in% fig1edxlist, c(1:24)]
```

```{r, fig.asp = 0.2}
fig2hm <- Heatmap(as.matrix(fig1edata), 
        ...
        grid.text(format(as.numeric(sprintf("%.2f", fig1edataNums[i, j])), nsmall = 2), x, y, gp = gpar(fontsize = 8))}
        )
...
```{r}
library(gridExtra)
grobFig1 <- annotate_figure(fig1, left = text_grob("Number of patients", rot = 90, size = 10), bottom = text_grob("Month of diagnosis documentation", size = 10))
grobFig2 <- grid.grabExpr(ComplexHeatmap::draw(fig2hm, heatmap_legend_side = "bottom"))
Fig2label <- textGrob("E", x = unit(0.032, "npc"), y = unit(0.5, "npc"), just = "left", gp = gpar(fontsize = 14, fontface = 'bold'))
```

```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/main-figs/figure-1.pdf", width = 9.5, height = 6)

grid.draw(grid.arrange(grobFig1, Fig2label, grobFig2,
                   widths = c(0.08, 1.25),
...
fig2b <- ggplot(all_sp_df3, aes(x = PostSpr2020_wtd, y = Apr2020, label = DiagnosisCategory)) +
...
```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/main-figs/figure-2.pdf", width = 8.5, height = 8, onefile = FALSE)

ggarrange(ggplotGrob(ggdraw(fig2a)), ggplotGrob(fig2b), ncol = 1, nrow = 2, labels = c("A", "B"), heights = c(0.7, 1))

dev.off()
```

```{r}
#lm(all_sp_df$PostSpr2020_wtd ~ all_sp_df$Apr2020)
#cor.test(all_sp_df3$PostSpr2020_wtd, all_sp_df3$Apr2020)
```
...
big4_sp_df <- merge(big4_sp_df, vehss_idx[, c(1,3)], by.x = "entity_name", by.y = "DiagnosisEntity", all.x = TRUE)
...
fig3excldxlist <- c("iridodialysis", "DME resolved after tx", "glaucoma, absolute", "glaucoma, PXG", "aphakia", "wet AMD, inactive scar", "lens dislocation", "PDR, stable", "IOL displacement")
```

```{r, fig.width=5, fig.height=2}
fig3a <- ggplot(big4_sp_df[big4_sp_df$entity_name %in% accurate_dx_list, ], aes(x = meanAllpost, y = `2020-04-01`, color = dx_category_final, label = ifelse(stringr::str_detect(entity_name, "other|unspec") | nchar(entity_name) > 25 | entity_name %in% fig3excldxlist, "", entity_name), size = mean_monthly_pts)) + geom_point(alpha = 0.4) + geom_text_repel(max.overlaps = Inf, size = 3, box.padding = 0.15, segment.alpha = 0.4, min.segment.length = 0, force_pull = -0.0075, force = 3) + geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + geom_abline(intercept = 0, slope = 1, color="grey", linetype="dashed") + guides(color = guide_legend(override.aes = aes(label = ""), order = 1)) + scale_size_area() + labs(title = "Deviations over time for common eye diseases: DR, AMD, cataract, glaucoma", y = "Deviation from Expectation, Apr '20", x = "Deviation from Expectation, Jun '20 - Dec '21", size = "Average monthly volume", color = "Diagnosis category") + annotate("text", x=-0.5, y=-0.05, label="r = 0.025", size = 3.25, col = 'blue') + theme_bw() #+ scale_x_continuous(expand = expansion(mult = 0.1), limits = c(-0.6, 0.4)) + + scale_y_continuous(expand = expansion(mult = 0.1)) 
```
...
```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/main-figs/figure-3.pdf", width = 9, height = 8, onefile = FALSE)

ggarrange(ggplotGrob(ggdraw(fig3a)), ggplotGrob(fig3b), ncol = 1, nrow = 2, labels = c("A", "B"), heights = c(0.5, 0.5))
...
fig4Ahyptest <- tidyr::gather(obj3_final_b, timeperiod, deviation, c(meanAllpost,`2020-04-01`), factor_key=TRUE)
fig4Ahyptest$deviation <- as.numeric(fig4Ahyptest$deviation)
fig4Ahyptest <- unique(fig4Ahyptest[, c('entity_name', 'MEDIAN_WEIGHT', 'timeperiod', 'deviation')])
```

### 4A: Ocular emergencies

*Kruskal-Wallis rank sum test:*
```{r}
kruskal.test(deviation ~ as.factor(MEDIAN_WEIGHT), data = fig4Ahyptest[fig4Ahyptest$timeperiod == '2020-04-01', ])
```

```{r}
kruskal.test(deviation ~ as.factor(MEDIAN_WEIGHT), data = fig4Ahyptest[fig4Ahyptest$timeperiod == 'meanAllpost', ])
```
...
```{r}
summary(glm(as.numeric(deviation) ~ as.numeric(MEDIAN_WEIGHT), data = fig4Ahyptest[fig4Ahyptest$timeperiod == '2020-04-01', ], family = "gaussian"))
```

```{r}
summary(glm(as.numeric(deviation) ~ as.numeric(MEDIAN_WEIGHT), data = fig4Ahyptest[fig4Ahyptest$timeperiod == 'meanAllpost', ], family = "gaussian"))
```

*Logistic regression: *

```{r}
fig4Ahyptest$severity_rank <- relevel(as.factor(fig4Ahyptest$MEDIAN_WEIGHT), ref = "1")
```

```{r}
summary(glm(as.numeric(deviation) ~ severity_rank, data = fig4Ahyptest[fig4Ahyptest$timeperiod == '2020-04-01', ], family = "gaussian"))
```

```{r}
summary(glm(as.numeric(deviation) ~ severity_rank, data = fig4Ahyptest[fig4Ahyptest$timeperiod == 'meanAllpost', ], family = "gaussian"))
```

```{r, fig.width=5, fig.height=2}
fig4a <- ggplot(fig4Ahyptest, aes(x = as.factor(MEDIAN_WEIGHT), y = as.numeric(deviation), fill = timeperiod)) + geom_boxplot(position = 'dodge') + labs(title = paste('Deviations from expectation vs. condition severity for', length(unique(fig4Ahyptest$entity_name)),'ocular emergencies'), x = 'Severity Index (BaSe SCOrE)', y = 'Deviation from Expectation', fill = "Time period") + scale_fill_manual(labels = c("Jun '20 - Dec '21", "Apr '20"), values = c("lightblue", "lightsalmon")) + ylim(c(-1, 0.25)) + geom_hline(yintercept = 0) + theme_bw()

fig4a <- fig4a + scale_x_discrete(breaks = c(1:5), labels = seq(1, 5, 1)) + geom_bracket(inherit.aes = F, xmin = 0.82, xmax = 4.82, y.position = 0.1, label = "Kruskal-Wallis test, p = 0.05", color = "dark grey", tip.length = 0.02) + geom_bracket(xmin = 1.18, xmax = 5.18, y.position = -0.9, inherit.aes = F, label = "", color = "dark grey", tip.length = -0.02) + annotate("text", label = "Kruskal-Wallis test, p = 0.002", y = -0.865, x = 2.75, color = "dark grey")
...
```{r}
fig4Ahyptest %>%
  group_by(MEDIAN_WEIGHT) %>%
  summarise(avg = mean(deviation),
            sd = sd(deviation),
...
```{r}
fig4Ahyptest[fig4Ahyptest$timeperiod == '2020-04-01', ] %>%
  group_by(MEDIAN_WEIGHT) %>%
  summarise(avg = mean(deviation),
            sd = sd(deviation),
            med = median(deviation),
...
```{r}
fig4Ahyptest[fig4Ahyptest$timeperiod == 'meanAllpost', ] %>%
  group_by(MEDIAN_WEIGHT) %>%
  summarise(avg = mean(deviation),
            sd = sd(deviation),
            med = median(deviation),
...
fig4Bhyptest <- tidyr::gather(obj3_final_v[obj3_final_v$entity_name %in% accurate_dx_list,], timeperiod, deviation, c(meanAllpost,`2020-04-01`), factor_key=TRUE)
fig4Bhyptest$deviation <- as.numeric(fig4Bhyptest$deviation)
fig4Bhyptest <- unique(fig4Bhyptest[, c("entity_name", "Big4Category", "Type", "timeperiod", "deviation")]) #doesn't do anything?
```

```{r}
#reclassify "PDR, stable", "DME resolved after tx", "glaucoma, OAG", "glaucoma, PXG" as NVT
fig4Bhyptest$Type <- ifelse(fig4Bhyptest$entity_name ==  "PDR, stable" | fig4Bhyptest$entity_name == "DME resolved after tx" | fig4Bhyptest$entity_name == "glaucoma, OAG" | fig4Bhyptest$entity_name == "glaucoma, PXG", "NVT", fig4Bhyptest$Type)

#exclude "glaucoma, other/unspec"
fig4Bhyptest <- fig4Bhyptest[fig4Bhyptest$entity_name != "glaucoma, other/unspec", ]
```

```{r}
fig4b <- ggplot(fig4Bhyptest, aes(x = as.factor(Type), y = as.numeric(deviation), fill = timeperiod)) +  facet_wrap(~Big4Category) + geom_boxplot(position = 'dodge') + labs(title = paste('Deviations from expectation vs. VT status for DR, AMD, and glaucoma diagnoses'), x = 'Vision-Threatening Status', y = 'Deviation from Expectation', fill = "Time period") + geom_hline(yintercept = 0) + theme_bw() + scale_fill_manual(labels = c("Jun '20 - Dec '21", "Apr '20"), values = c("lightblue", "lightsalmon"))
fig4b
```

```{r}
fig4Bstattest <- compare_means(deviation ~ Type, data = fig4Bhyptest, group.by = c('Big4Category', 'timeperiod'))
fig4Bstattest <- fig4Bstattest[order(fig4Bstattest$Big4Category), ]
fig4Bstattest
```
...
```{r, fig.width=5, fig.height=2}
fig4b <- fig4b + geom_bracket(data = plabels,
...
```
```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/main-figs/figure-4.pdf", width = 9, height = 8, onefile = FALSE)

ggarrange(fig4a, fig4b, ncol = 1, nrow = 2, labels = c("A", "B"), heights = c(0.5, 0.5), common.legend = T, legend = "bottom")
...
```{r}
ttr_func <- function(entity_name, input_df){
  
  df <- input_df[rownames(input_df) == entity_name, ]
  df_long <- df %>% gather(timepoint)
  df_long <- df_long[df_long$value >= 0, ]
  tp_indicies <- as.numeric(rownames(df_long[df_long$value >= 0, ]))
  tp_indicies <- tp_indicies[tp_indicies > 5]
  
  res_df <- as.data.frame(seqToIntervals(tp_indicies))
...
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/main-figs/figure-5.pdf")
...
circos.heatmap.initialize(master_summarystats[, rev(c("Q32020w", "Q42020w", "Q12021w", "Q22021w", "Q32021w", "Q42021w"))], split = master_summarystats$short_name)
...
circos.heatmap(master_summarystats[, rev(c("Q32020w", "Q42020w", "Q12021w", "Q22021w", "Q32021w", "Q42021w"))], 
...
# Supplemental / exploratory figures

### FIGURE S1: Deviations faceted by category

```{r}
#define a function to compute the mean Mahalanobis distance given two dimensions (deviations/summary devs) 
mhbDist <- function(dev1, dev2, dx_cat){
  
  df <- master_summarystats[master_summarystats$Category == dx_cat, c(dev1, dev2)]
...
```{r}
#define a function to compute the mean pairwise Euclidean distances amongst all points in the cluster
...
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s1.pdf", width = 9, height = 8, onefile = FALSE)
...
```{r}
retinal_dx_list <- master_summarystats[master_summarystats$Category == 'Retinal and vitreous conditions', 'entity_name'] 
...
```{r, fig.width=5, fig.height=2}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s2.pdf", width = 9.25, height = 4, onefile = FALSE)
...
```{r}
corneal_dx_list <- master_summarystats[master_summarystats$Category == 'Cornea and external disease', 'entity_name']
...
corneal_top10_crosswalk$Top10 <- ifelse(corneal_top10_crosswalk$Diagnosis.Name == "other conjunctival disorders", "	
Other disorders of conjunctiva", corneal_top10_crosswalk$Top10)
...
##View(table(corneal_sp_df$Top10))
...
```{r, fig.width=5, fig.height=2.25}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s3.pdf", width = 9.25, height = 4.25, onefile = FALSE)
...
```{r}
oculoplast_dx_list <- master_summarystats[master_summarystats$Category == 'Oculofacial plastics and orbital conditions', 'entity_name']
...
##View(table(oculoplast_sp_df$Top10))
...
```{r, fig.width=5, fig.height=2.25}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s4.pdf", width = 9.25, height = 4.25, onefile = FALSE)
...
```{r}
uveitis_dx_list <- master_summarystats[master_summarystats$Category == 'Uveitis and ocular inflammation', 'entity_name']
...
##View(table(uveitis_sp_df$Top10))
...
```{r, fig.width=5, fig.height=2}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s5.pdf", width = 9.25, height = 4.25, onefile = FALSE)
...
```{r}
blindvdf_dx_list <- master_summarystats[master_summarystats$Category == 'Blindness and vision defects', 'entity_name']
...
```{r, fig.width=5, fig.height=2}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s6.pdf", width = 9.25, height = 4.25, onefile = FALSE)
...
## TABLE S1: Select conditions post-hiatus
...
names(sTables_colors) <- unique(master_summarystats$short_name)
#sTables_colorscale <- scale_fill_manual(name = "Diagnosis Category", values = sTables_colors)
```

#### S1A: Suppression post-hiatus
...
#only include all conditions whose post-pandemic deviation <= -0.20 and do not have any recovery
...
#### S1B: Met or exceeded expectation post-hiatus
Conditions that met or exceed counterfactual expectation in the post-hiatus period (mean deviation ≥ 0%). 
...
```
```{r}
#to create a common legend, generate a dummy ggplot containing the dx entities in both tblS1A and tblS1B
...
```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/table-s1.pdf", width = 9, height = 8, onefile = FALSE)

ggarrange(tblS1A, tblS1B, ncol = 1, nrow = 2, labels = c("A", "B"), heights = c(0.65, 0.35), legend = "right", legend.grob = get_legend(figS1X))

dev.off()
```

## TABLE S2: Rebounded conditions
Conditions that exhibited any type of rebound (full or partial): 
```{r}
...
```
```{r}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/table-s2.pdf", width = 7.5, height = 12, onefile = FALSE)
...
## Master heatmap of all deviances (retaining the actual value of nonsigs, and shading based on alpha)
...
```{r, fig.asp = 3}
pdf(file="/Users/charlesli/Library/CloudStorage/OneDrive-AmericanAcademyofOphthalmology/covid-elasticity-paper/outputs/06072022/suppl-figs/figure-s7.pdf", width = 12, height = 36)
